...
export class RepairOverveiwComponent implements OnInit {
  ...
  reviewFormSubmit() {
   ...
   let repairDates = this.repairObj.formObj.value.pickerModel;
    ...
    this.list.forEach(element => {
      console.log(element);
      console.log(this.repairObj.formObj.value.repairStation);
      if (element.id.toString() === this.repairObj.formObj.value.repairStation.toString()) {
        repairStationVal = element;
      }
    });
   let removalDate = this.createRemovalObj.formObj.value.pickerModel;
    ...
    const finalPostObj = {
      ...
      repairTechnician: this.repairObj.formObj.value.techician,
      workOrder: this.repairObj.formObj.value.workOrder,
      noFaultFound: this.faultObj.formObj.value.remarks,
      removalId: '',
      createdAt: '',
      updatedAt: '',
      repairStation: repairStationVal,
      removal: {
        ...
        removalDate: this.allModelData['removalDate'], //removalDate,
        maintenanceStationId: this.allModelData['maintenanceStationId'],//this.createRemovalObj.formObj.value.maintenanceSation,
        lruPartNumberId: this.allModelData['lruPartNumberId'],//this.createRemovalObj.formObj.value.repairlruPartNumber,
        serialNumberOFF:this.allModelData['serialNumberOFF'], //this.createRemovalObj.formObj.value.lruSerialNumber,
        serialNumberON: '',
        aircraftId: this.allModelData['aircraftId'],//this.createRemovalObj.formObj.value.repairTail,
        modDotIn: this.allModelData['modDotIn'],//this.empMod.toString(),
        otherReasonOfRemoval: '',
        revision: this.allModelData['revision'],//getRevision,
        createdAt: '',
        updatedAt: ''
      }
    };
    console.log(this.createRemovalObj.formObj.value.maintenanceSation,'================> maintenanceStation');

    this.repiarsService.createRepair(finalPostObj).subscribe((data) => {
      this.repairData = data;
      console.log(this.repairData, '===================>finalobject');
      if (this.repairData['modDotOut']) {
        this.modOutData = this.repairData['modDotOut'].split(',').map(i => {
          return parseInt(i, 10);
        });
      }
      // this.repairObj.formObj.value.pickerMode = repairDate;
    });
  }
  modFormSubmit() {
    this.modObj.formSubmitted = true;
  ...
  selectedBtn(id) {
      console.log( 'index--->>>', this.empMod.indexOf(id));
